...
This project manages Home Assistant configuration files with automated validation, testing, and deployment.
...
- `Taskfile.yaml` - Commands for pulling/pushing configuration
...
- `task pull` - Pull latest config from Home Assistant instance
- `task push` - Push local config to Home Assistant (with validation)
- `task backup` - Create backup of current config
...
- `uv run python tools/entity_explorer.py` - Entity registry parser and explorer
...
- **Host**: Configure in .env `HA_HOST` variable
...
1. **Pull Latest**: `task pull` to sync from HA
2. **Edit Locally**: Modify files in `config/` directory
3. **Auto-Validation**: Hooks automatically validate on edits
4. **Test Changes**: `task validate` for full test suite
5. **Deploy**: `task push` to upload (blocked if validation fails)
...
- All python tools need to be run with  `uv run python <tool_path>`
